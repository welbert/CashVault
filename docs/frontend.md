...
- **Sizes** — `CardSize` is one of 18 values (`"1x1"` .. `"6x3"`), generated into `SIZE_DIMENSIONS` (`catalog.ts`) as `{w, h}`: width is a direct column count (1..`GRID_COLS = 6`), height in units of 1 row (180px, `ROW_HEIGHT` in `DashboardGrid.tsx`). Each catalog entry's `allowedSizes` is the subset that actually fits its content (e.g. donut+legend cards are narrow-tall `2x2`/`2x3`, never wide — a wide `4x2` for those wastes both width and height; the flow chart is the opposite, `6xN` only). Resizing always snaps to the nearest *allowed* size (`gridConstraints.ts`'s `allowedSizeConstraint`, a `react-grid-layout` `LayoutConstraint`) — never freeform. The width numbering changed once (was 2-column steps `1/2/3` → `2/4/6`, now direct columns `1..6`) — pre-existing saved layouts are translated by a one-time, self-guarded data migration in `db.rs`'s `migrate_dashboard_layout_size_scheme`.
